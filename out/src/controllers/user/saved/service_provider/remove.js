...
exports.default = async (req, res) => {
    ...
    let where = {
        user: req.auth.id,
        service_provider: parseInt(req.query.id)
    };
    try {
        toBeDeleted = await prisma_1.default.client.saved_service_provider.findFirst({ where });
        if (!toBeDeleted)
    ...
    try {
        await prisma_1.default.client.saved_service_provider.deleteMany({ where });
