...
exports.default = async (req, res) => {
    ...
    let where = {
        user: req.auth.id,
        news: parseInt(req.query.id)
    };
    try {
        toBeDeleted = await prisma_1.default.client.saved_news.findFirst({ where });
        if (!toBeDeleted)
    ...
    try {
        await prisma_1.default.client.saved_news.deleteMany({ where });
