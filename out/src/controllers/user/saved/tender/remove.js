...
exports.default = async (req, res) => {
    ...
    let where = {
        user: req.auth.id,
        tender: parseInt(req.query.id)
    };
    try {
        toBeDeleted = await prisma_1.default.client.saved_tender.findFirst({ where });
        if (!toBeDeleted)
    ...
    try {
        await prisma_1.default.client.saved_tender.deleteMany({ where });
