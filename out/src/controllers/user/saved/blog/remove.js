...
exports.default = async (req, res) => {
    ...
    let where = {
        user: req.auth.id,
        blog: parseInt(req.query.id)
    };
    try {
        toBeDeleted = await prisma_1.default.client.saved_blog.findFirst({ where });
        if (!toBeDeleted)
    ...
    try {
        await prisma_1.default.client.saved_blog.deleteMany({ where });
