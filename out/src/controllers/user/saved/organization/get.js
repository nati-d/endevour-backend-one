...
exports.default = async (req, res) => {
    try {
        ...
        let data = await prisma_1.default.client.saved_organization.findMany({
            take: 10,
            skip: page,
            where: { user: req.auth.id },
        });
        let totalData = await prisma_1.default.client.saved_organization.count({ where: { user: req.auth.id } });
