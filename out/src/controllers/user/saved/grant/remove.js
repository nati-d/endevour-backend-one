...
exports.default = async (req, res) => {
    ...
    let where = {
        user: req.auth.id,
        grant: parseInt(req.query.id)
    };
    try {
        toBeDeleted = await prisma_1.default.client.saved_grant.findFirst({ where });
        if (!toBeDeleted)
    ...
    try {
        await prisma_1.default.client.saved_grant.deleteMany({ where });
