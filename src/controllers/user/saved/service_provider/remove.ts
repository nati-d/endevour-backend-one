...
export default async (req: Request, res: Response) => {
    ...
    let where = {
        user: req.auth.id,
        service_provider: parseInt( req.query.id as string )
    }
    try {
        toBeDeleted = await prisma.client.saved_service_provider.findFirst({ where });
    ...
    try {
        await prisma.client.saved_service_provider.deleteMany({ where });
