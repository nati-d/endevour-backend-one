...
export default async (req: Request, res: Response) => {
    ...
    let where = {
        user: req.auth.id,
        organization: parseInt( req.query.id as string )
    }
    try {
        toBeDeleted = await prisma.client.saved_organization.findFirst({ where });
    ...
    try {
        await prisma.client.saved_organization.deleteMany({ where });
